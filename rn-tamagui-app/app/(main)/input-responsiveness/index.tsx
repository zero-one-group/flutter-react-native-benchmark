...
import { Stack, Image, View, ScrollView } from "tamagui";
...
const initValues: Item[] = Array.from({ length: COUNT_ITEMS }).map(
  (_, index) => {
    return {
      index,
...
export default function InputResponsiveness() {
  ...
  const backgroundColor = (index: number) =>
    animationValue.interpolate({
      inputRange: [0, 1],
      outputRange:
        index % 2 === 0 ? ["#FF6969", "#6987FF"] : ["#6987FF", "#FF6969"],
    });
      ...
      >
        <Animated.View>
          <Animated.Text style={{ color: "white", fontWeight: "500" }}>
            {isShow ? Words.hideList : Words.showList}
          </Animated.Text>
        </Animated.View>
      </Stack>
      <View style={styles.imageContainer}>
        <Animated.View style={{ transform: [{ rotate: spin }] }}>
          <Image objectFit="cover" source={bgImage} style={styles.image} />
        </Animated.View>
      </View>
      {isShow && (
